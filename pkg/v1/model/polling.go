...
type PollQueueStatistics struct {
	...
	// Statistics
	MaxPollExecuteTimeSecs        float64 `json:"max_poll_execute_time_secs"`        // time in seconds for polling to complete (poll response time, doesn't include the time in queue).
	AveragePollExecuteTimeSecs    float64 `json:"average_poll_execute_time_secs"`    // time in seconds for polling to complete (poll response time, doesn't include the time in queue).
	MinPollExecuteTimeSecs        float64 `json:"min_poll_execute_time_secs"`        // time in seconds for polling to complete (poll response time, doesn't include the time in queue).
	...
	ASAPPriorityAveragePollTime   float64 `json:"asap_priority_average_poll_time"`   // average time in seconds between ASAP priority polling point added to current queue, and polling complete.
	HighPriorityAveragePollTime   float64 `json:"high_priority_average_poll_time"`   // average time in seconds between High priority polling point added to current queue, and polling complete.
	NormalPriorityAveragePollTime float64 `json:"normal_priority_average_poll_time"` // average time in seconds between Normal priority polling point added to current queue, and polling complete.
	LowPriorityAveragePollTime    float64 `json:"low_priority_average_poll_time"`    // average time in seconds between Low priority polling point added to current queue, and polling complete.
	...
	BusyTime                      float64 `json:"busy_time"`                         // percent of the time that the plugin is actively polling.
	EnabledTime                   float64 `json:"enabled_time"`                      // time in seconds that the statistics have been running for.
	PortUnavailableTime           float64 `json:"port_unavailable_time"`             // time in seconds that the serial port has been unavailable.
}
